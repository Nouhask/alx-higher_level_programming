...
/**
 * is_palindrome - the function to call check_pal to see if the list is palindrome.
...
/**
 * check_pal - a function to check if the list is palindrome.
 * @head: ptr to the beginning of the list.
...
int check_pal(listint_t **head, listint_t *last)
{
	if (last == NULL)
		return (1);
	if (check_pal(head, last->next) && (*head)->k == last->k)
	{
		*head = (*head)->next;
		return (1);
	}
